...
const makeSelectNewRequestList = () => {
  return createSelector(
    selectRequestDomain,
    substate => substate.requests ? substate.requests.filter(request => request.campaignStatusId === CampaignStatus.PEDNING) : []
  );
}
...
const makeSelectInProgressRequestList = () => {
  return createSelector(
    selectRequestDomain,
    substate => substate.requests ? substate.requests.filter(request => (request.campaignStatusId === CampaignStatus.ACCEPTED || request.campaignStatusId === CampaignStatus["IN-PROGRESS"])) : []
  );
}
...
const makeSelectCompletedRequestList = () => {
  return createSelector(
    selectRequestDomain,
    substate => substate.requests ? substate.requests.filter(request => (request.campaignStatusId === CampaignStatus.COMPLETED || request.campaignStatusId === CampaignStatus.APPROVED)) : []
  );
}
