...
const _renderGenres = (genersToRender, genres) => {
  return (
    <div style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
      {genersToRender && isArray(genersToRender) && (genersToRender || []).map((internalGener) => {
        return (
          <div style={{
            ...
            <div style={{
              ...
              color: 'white'
            }}>{(genres.find(gener => gener.id === internalGener.genreId) || {}).title}</div>
...
function MyAccount({
  ...
  return (
    <div className="container-fluid" style={{ marginTop: '100px' }}>
      ...
      <div style={{ margin: 20, marginLeft: 10, display: 'flex', flexDirection: 'row', width: '100%' }}>
        <div style={{ borderWidth: 0, borderRightWidth: 2, borderColor: 'green', borderStyle: 'solid', flex: 1 }}>
          <div><>Social medias</><Link to={{pathname: '/requestInfluencer', param: influencerProfile, fromEdit: true}}><FontAwesomeIcon
            size="x"
        ...
        <div style={{ flex: 1 }}>
          <div style={{ marginLeft: 10 }}>Genres<FontAwesomeIcon
            size="x"
            color={PLAY_ICON_BG_COLOR}
            icon={faEdit}
            style={{ marginLeft: 5 }}
